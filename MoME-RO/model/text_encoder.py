...
class TextContextEncoder(nn.Module):
    ...
    def forward(self, text, context=None, train=False, alpha=None):
        ...
        if train & self.noise:
            noise_ = torch.zeros_like(x_text).uniform_(-1,1)
            dims = x_text.size(-1)
            if alpha is not None:
                mag = (10.0 + alpha) / torch.sqrt(torch.tensor(dims)) #self.alpha 20.0 10.0 5.0
            else:
                mag = 10.0 / torch.sqrt(torch.tensor(dims)) #self.alpha 20.0 10.0 5.0
            noise_ = (noise_ * mag).detach()
            x_text = x_text + noise_
        ...
        if context is not None:
            # if self.all:
            #     P = 1
            #     x = torch.cat([x_text[:,0:1], context, x_text[:, 1:]], dim=1)
            # else:
        ...
        if self.llm:
            ...
            if self.all:
                x = rearrange(x, '(p b) l c -> (b p) l c', b=B, p=P)
                return x
