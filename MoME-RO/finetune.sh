...
do
    ...
    if [[ $CHECK == *"multimodal"* ]] ; then 
        ...
        CUDA_VISIBLE_DEVICES=$GPU_DEVICE python main.py --target 2 --gtv_dir $WORKDIR_GTV$CHECK_GTV --pretrained_dir $SAVEDIR$CHECK --context True --n_prompts 1 --context_length 32 --test_mode 4 --force_expert 2 --flag_pc True --textencoder 'llama3' 
    else
        ...
        CUDA_VISIBLE_DEVICES=$GPU_DEVICE python main.py --target 2 --gtv_dir $WORKDIR_GTV$CHECK_GTV --pretrained_dir $SAVEDIR$CHECK --test_mode 4 --flag_pc True 
    fi
done
